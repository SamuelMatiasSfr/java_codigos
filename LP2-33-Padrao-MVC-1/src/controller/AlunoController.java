...
public class AlunoController {	
	...
	public void setListeners() {
		...
		view.addListenerAddButton(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				try{
					...
					view.showMessage("Matrícula inválida");
				}
		...
		for (int aux = 0; aux < lista.size(); aux++) {
			...
			lista.get(i).addListenerUpdateButton(new ActionListener() {
				private int atualMatricula;

				@Override
				public void actionPerformed(ActionEvent e) {
					if(lista.get(i).getButtonEditar().getText().equals("Editar")){
						this.atualMatricula = lista.get(i).getMatricula();
						...
						updateAluno(atualMatricula, lista.get(i).getMatricula(), lista.get(i).getName());
	...
	public void updateAluno(int atualMatricula, int novaMatricula, String nome) {
		ArrayList<Aluno> alunos = model.getAlunos();
		for(int i=0; i<alunos.size(); i++){
			if(alunos.get(i).getMatricula() == atualMatricula){
				alunos.get(i).setMatricula(novaMatricula);
